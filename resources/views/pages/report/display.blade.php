...
@section('title', 'View Registration Certificate')
...
<div class="add-heading">
    <h3 class="heading-cm">
        ...
        Certificate<span class="sub-nav ms-2"> > Registration > 
...
<div class="product-search-form mb-3">
   <div class="row log-details">
        ...
        @if($report->pending_id != null)
        <div class="col-md-3">
            <div class="row">
                <p>Status changed by</p>
                <h4>{{ $report->pending->name }}</h4>
            </div>
            <div class="row mt-2">
                <p>Status changed at</p>
                <h4>{{ $report->pending_at }}</h4>
            </div>
        </div>
        @else
        ...
        @if($report->verifier_id != null)
        <div class="col-md-3">
            <div class="row">
                <p>Verified By</p>
                <h4>{{ $report->verifier->name }}</h4>
            </div>
            <div class="row mt-2">
                <p>Rejected at</p>
                <h4>{{ $report->verified_at }}</h4>
            </div>
        </div>
        @else
        <div class="col-md-3"></div>
        ...
        <div class="col-md-3 log-status">
            <div class="@if($report->status == 'Pending') pending @elseif($report->status == 'Verified') verified @else rejected @endif">{{ $report->status}}</div>
        </div>
   </div>
</div>
...
<h3 class="create-form-heading">Certificate Details</h3>
        <div class="row">
            ...
            <div class="col-md-4">
                ...
                <p>{{ $report->application_number }}</p>
            </div>
            <div class="col-md-4">
                <h4>Date of Grant</h4>
                <p>{{ $report->date_of_grant}}</p>
            </div>
        </div>
        <div class="row">
            <div class="col-md-4">
                <h4>Certifi Validity From</h4>
                <p>{{ $report->validity_from }}</p>
            </div>
            <div class="col-md-4">
                <h4>Certificate Valid To</h4>
                <p>{{ $report->validity_to }}</p>
            </div>
            <div class="col-md-4">
                <h4>GMP Validity Upto</h4>
                <p>{{ $report->gmp_validity}}</p>
            </div>
        </div>
        <div class="row">
            <div class="col-md-4">
            ...
            <div class="col-md-4">
                ...
                <p>{{ $report->date_of_preparation}}</p>
            </div>
        </div>
</div>
